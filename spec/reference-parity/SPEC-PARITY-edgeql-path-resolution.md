# SPEC-PARITY: Path scoping (`edgeql/path_resolution.rst`)
...
Define one-for-one parity targets for `edgeql/path_resolution.rst` so sqlite-ts can match Gel's documented feature set.
...
- [ ] S1: `New path scoping` parity is implemented and covered by tests.
- [ ] S2: `Path scoping configuration` parity is implemented and covered by tests.
- [ ] S3: `Warning on old scoping` parity is implemented and covered by tests.
- [ ] S4: `Recommended upgrade plan` parity is implemented and covered by tests.
- [ ] S5: `Legacy path scoping` parity is implemented and covered by tests.
- [ ] S6: `Scopes` parity is implemented and covered by tests.
- [ ] S7: `Clauses & Nesting` parity is implemented and covered by tests.

## Requirements

- R1: sqlite-ts SHALL parse and/or execute constructs required by `edgeql/path_resolution.rst` with equivalent user-visible behavior.
...
- Upstream docs:
  - `docs/reference/edgeql/path_resolution.rst`
