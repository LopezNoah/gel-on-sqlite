# SPEC-PARITY: Triggers (`datamodel/introspection/triggers.rst`)
...
Define one-for-one parity targets for `datamodel/introspection/triggers.rst` so sqlite-ts can match Gel's documented feature set.
...
- R1: sqlite-ts SHALL parse and/or execute constructs required by `datamodel/introspection/triggers.rst` with equivalent user-visible behavior.
...
- Upstream docs:
  - `docs/reference/datamodel/introspection/triggers.rst`
