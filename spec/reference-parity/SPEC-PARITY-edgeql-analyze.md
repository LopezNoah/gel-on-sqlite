# SPEC-PARITY: Analyze (`edgeql/analyze.rst`)
...
Define one-for-one parity targets for `edgeql/analyze.rst` so sqlite-ts can match Gel's documented feature set.
...
- R1: sqlite-ts SHALL parse and/or execute constructs required by `edgeql/analyze.rst` with equivalent user-visible behavior.
...
- Upstream docs:
  - `docs/reference/edgeql/analyze.rst`
