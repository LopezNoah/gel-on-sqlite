# SPEC-PARITY: Functions (`datamodel/introspection/functions.rst`)
...
Define one-for-one parity targets for `datamodel/introspection/functions.rst` so sqlite-ts can match Gel's documented feature set.
...
- R1: sqlite-ts SHALL parse and/or execute constructs required by `datamodel/introspection/functions.rst` with equivalent user-visible behavior.
...
- Upstream docs:
  - `docs/reference/datamodel/introspection/functions.rst`
