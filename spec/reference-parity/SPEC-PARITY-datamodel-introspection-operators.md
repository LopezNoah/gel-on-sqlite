# SPEC-PARITY: Operators (`datamodel/introspection/operators.rst`)
...
Define one-for-one parity targets for `datamodel/introspection/operators.rst` so sqlite-ts can match Gel's documented feature set.
...
- R1: sqlite-ts SHALL parse and/or execute constructs required by `datamodel/introspection/operators.rst` with equivalent user-visible behavior.
...
- Upstream docs:
  - `docs/reference/datamodel/introspection/operators.rst`
