# SPEC-PARITY: For (`edgeql/for.rst`)
...
Define one-for-one parity targets for `edgeql/for.rst` so sqlite-ts can match Gel's documented feature set.
...
- [ ] S1: `Bulk inserts` parity is implemented and covered by tests.
- [ ] S2: `Conditional DML` parity is implemented and covered by tests.

## Requirements

- R1: sqlite-ts SHALL parse and/or execute constructs required by `edgeql/for.rst` with equivalent user-visible behavior.
