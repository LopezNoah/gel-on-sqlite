# SPEC-PARITY: Types (`edgeql/types.rst`)
...
Define one-for-one parity targets for `edgeql/types.rst` so sqlite-ts can match Gel's documented feature set.
...
- [ ] S1: `Type expressions` parity is implemented and covered by tests.
- [ ] S2: `Type casting` parity is implemented and covered by tests.
- [ ] S3: `Type intersections` parity is implemented and covered by tests.
- [ ] S4: `Type checking` parity is implemented and covered by tests.
- [ ] S5: `The ``typeof`` operator` parity is implemented and covered by tests.
- [ ] S6: `Introspection` parity is implemented and covered by tests.

## Requirements

- R1: sqlite-ts SHALL parse and/or execute constructs required by `edgeql/types.rst` with equivalent user-visible behavior.
...
- Upstream docs:
  - `docs/reference/edgeql/types.rst`
