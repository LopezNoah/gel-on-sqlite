# SPEC-PARITY: Indexes (`datamodel/introspection/indexes.rst`)
...
Define one-for-one parity targets for `datamodel/introspection/indexes.rst` so sqlite-ts can match Gel's documented feature set.
...
- R1: sqlite-ts SHALL parse and/or execute constructs required by `datamodel/introspection/indexes.rst` with equivalent user-visible behavior.
...
- Upstream docs:
  - `docs/reference/datamodel/introspection/indexes.rst`
