# SPEC-PARITY: Transactions (`edgeql/transactions.rst`)
...
Define one-for-one parity targets for `edgeql/transactions.rst` so sqlite-ts can match Gel's documented feature set.
...
- [ ] S1: `Client libraries` parity is implemented and covered by tests.
- [ ] S2: `TypeScript/JS` parity is implemented and covered by tests.
- [ ] S3: `Python` parity is implemented and covered by tests.
- [ ] S4: `Golang` parity is implemented and covered by tests.
- [ ] S5: `Rust` parity is implemented and covered by tests.

## Requirements

- R1: sqlite-ts SHALL parse and/or execute constructs required by `edgeql/transactions.rst` with equivalent user-visible behavior.
