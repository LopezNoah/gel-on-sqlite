# SPEC-PARITY: vs SQL and ORMs (`datamodel/comparison.rst`)
...
Define one-for-one parity targets for `datamodel/comparison.rst` so sqlite-ts can match Gel's documented feature set.
...
- [ ] S1: `Comparison to SQL` parity is implemented and covered by tests.
- [ ] S2: `Comparison to ORMs` parity is implemented and covered by tests.

## Requirements

- R1: sqlite-ts SHALL parse and/or execute constructs required by `datamodel/comparison.rst` with equivalent user-visible behavior.
...
- Upstream docs:
  - `docs/reference/datamodel/comparison.rst`
