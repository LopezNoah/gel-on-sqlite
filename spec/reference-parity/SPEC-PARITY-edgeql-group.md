# SPEC-PARITY: Group (`edgeql/group.rst`)
...
Define one-for-one parity targets for `edgeql/group.rst` so sqlite-ts can match Gel's documented feature set.
...
- R1: sqlite-ts SHALL parse and/or execute constructs required by `edgeql/group.rst` with equivalent user-visible behavior.
...
- Upstream docs:
  - `docs/reference/edgeql/group.rst`
