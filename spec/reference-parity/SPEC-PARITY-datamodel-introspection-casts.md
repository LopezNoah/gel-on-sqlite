# SPEC-PARITY: Casts (`datamodel/introspection/casts.rst`)
...
Define one-for-one parity targets for `datamodel/introspection/casts.rst` so sqlite-ts can match Gel's documented feature set.
...
- R1: sqlite-ts SHALL parse and/or execute constructs required by `datamodel/introspection/casts.rst` with equivalent user-visible behavior.
...
- Upstream docs:
  - `docs/reference/datamodel/introspection/casts.rst`
