# SPEC-PARITY: Schema (`datamodel/index.rst`)
...
Define one-for-one parity targets for `datamodel/index.rst` so sqlite-ts can match Gel's documented feature set.
...
- [ ] S1: `Schema Definition Language` parity is implemented and covered by tests.
- [ ] S2: `Migrations and DDL` parity is implemented and covered by tests.
- [ ] S3: `Instances, branches, and modules` parity is implemented and covered by tests.

## Requirements

- R1: sqlite-ts SHALL parse and/or execute constructs required by `datamodel/index.rst` with equivalent user-visible behavior.
