# SPEC-PARITY: Delete (`edgeql/delete.rst`)
...
Define one-for-one parity targets for `edgeql/delete.rst` so sqlite-ts can match Gel's documented feature set.
...
- [ ] S1: `Clauses` parity is implemented and covered by tests.
- [ ] S2: `Link deletion` parity is implemented and covered by tests.
- [ ] S3: `Cascading deletes` parity is implemented and covered by tests.
- [ ] S4: `Return value` parity is implemented and covered by tests.

## Requirements

- R1: sqlite-ts SHALL parse and/or execute constructs required by `edgeql/delete.rst` with equivalent user-visible behavior.
...
- Upstream docs:
  - `docs/reference/edgeql/delete.rst`
