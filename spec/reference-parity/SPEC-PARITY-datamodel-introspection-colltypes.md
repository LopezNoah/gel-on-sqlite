# SPEC-PARITY: Collection types (`datamodel/introspection/colltypes.rst`)
...
Define one-for-one parity targets for `datamodel/introspection/colltypes.rst` so sqlite-ts can match Gel's documented feature set.
...
- [ ] S1: `Array` parity is implemented and covered by tests.
- [ ] S2: `Tuple` parity is implemented and covered by tests.

## Requirements

- R1: sqlite-ts SHALL parse and/or execute constructs required by `datamodel/introspection/colltypes.rst` with equivalent user-visible behavior.
...
- Upstream docs:
  - `docs/reference/datamodel/introspection/colltypes.rst`
