...
1. WITH U := User { cards := Card } SELECT count((U.cards.name, U.cards.cost)) returns 81.
1. The same verdicts come directly from analyzeTreeFactoring; the tuple stamp and isWithBinding special gate are removed or become mere outputs of that single authority.
1. Pointer-prefix collapse deduplicates target IDs before counting.
1. scope_computables_07a, 07c, and 08 return 81, 0, and 16.
1. Scope-tree formatting uses real typed PathIds and matches Gel scope-tree goldens, not only sqlite-ts structural sig: names.
1. No factoring decision is inferred from successful Relation lookup.
...
1. Shadowed bindings with equal names receive different IDs.
1. Binding definitions preserve shape, filter, order, limit, and offset.
1. Unsupported binding shapes fail explicitly rather than degrading to a type root.
1. Repeated reads refer to the same semantic binding source while retaining EdgeQL’s intended independent-iteration/cross-product behavior.
1. Runtime-generated captures use the same IR representation.
1. The 9/81 factoring tests remain unchanged.
1. Filtered object bindings such as scope_computables_07c preserve their filter.
1. bindingAst and shape-reconstruction paths are removed once their facts are represented directly.
1. Scalar, tuple, array, FOR, GROUP, and free-object bindings remain behaviorally unchanged until explicitly migrated.
...
1. count(pointer.scalar) produces 2, not 1 or 3.
1. Selection, aggregate, predicate, and shape consumers all observe the same logical object set.
1. SQL performs identity dedup before scalar projection; no migrated branch emits SELECT DISTINCT scalar as the identity mechanism.
1. Raw edge rows do not escape pointer-row lowering.
1. Each migrated slice deletes its previous consumer-specific workaround, including namespace-based identity exceptions.
1. Physical SQL shape may differ by slice, but semantic ordering remains traversal → target-ID dedup → scalar projection.
   Relation migration
1. New range variables register all relevant path/aspect providers in Relation.
1. Migrated code resolves paths exclusively via Relation; removed channels such as outerScopes, sourcePathAliases, multiScalarBindings, and scopedAggRoot remain absent.
1. source, identity, iterator, value, and serialized aspects remain distinct.
1. Child scopes shadow parent scopes; recursive column injection is idempotent.
1. Aggregate lookup consumes only locally registered aggregate scope, not ordinary/enclosing scope.
1. FOR tuple slots, link properties, and GROUP projections remain construct-local.
1. A successful provider lookup never changes factoring or object-set semantics.
   Pointer-path implementation migration
1. Each migrated path family has one semantic implementation.
1. The adapter either preserves PathIds, compound types, carried shapes, and binding identity or rejects the family before interpretation.
1. No partial qlast interpretation followed by silent direct-compiler reinterpretation.
1. Differential tests compare full downstream-relevant facts, not only a pointer-chain signature.
1. Completion of a family includes deleting its redundant branch.
1. No new cyclic pointer_nav or shape_compiler module is introduced.
   ADR 0063 follow-up
1. planExclusiveConflictProbe accepts ScalarValue | ScalarValue[] | undefined.
1. The as unknown as ScalarValue cast in engine.ts is removed.
1. Existing single- and multi-property conflict-probe tests remain behaviorally unchanged.
