...
- **The link-storage boolean** `!multi && properties.length === 0` (= `!usesLinkTable`) was still inlined at two live sites ADR 0008's consolidation slice didn't reach: the link-table DDL in `runtime/database.ts:1075` (the very place junction tables are _created_) and inside `service.ts`'s column resolver.
