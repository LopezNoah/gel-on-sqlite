...
- **28 + 1** shape-element cardinality / type cases — these read per-shape-element facts the **SQL builder** constructs (`shape[].cardinality`, shape typerefs), a different concern from statement-level inference, and one that _is_ read by execution (so not additively changeable here). Tracked, not regressions.
