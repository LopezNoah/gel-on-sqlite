...
- **Inline in a computed shape** — `SELECT Card { foo := count(Card.name) }` —
  the `Card` inside the count is the _enclosing row_. The count is therefore
  per-row: `0` or `1` on that row's single `name`. (`test_edgeql_scope_nested_05`.)
- **Factored into a `WITH` binding** — `WITH a := count({Card.name})
SELECT Card { a := a }` — the binding is computed once over the _whole set_,
...
site, the two are _byte-identical_ in the IR: same `function_call`/`pointer`
...
scope the path was bound in) is _erased by inlining_.
...
- `withBindings` (ast_to_ir.ts): a `count(...)` WITH binding now stamps
  ...
  receives. The mark survives inlining and is the _only_ thing distinguishing a
...
NULL THEN 0 ELSE 1 END`. The raw column comes from a new
  ...
  `SqlLoweringContext`) — _raw_, not json-quoted, so the `IS NULL` test reads
...
declines the match for free — is read by _every_ correlation call site, a broad
...
**Scoped out, deliberately.** Only the single-valued _direct_ scalar property
...
— demonstrated to fail in/out across identical-code runs _and_ on the stashed
