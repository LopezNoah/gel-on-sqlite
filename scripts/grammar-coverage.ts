...
for (const { query } of corpus.queries) {
  ...
  try {
    ...
    const signature = message
      ...
      .replace(/Redundant input, expecting EOF but found: .*/g, "Redundant input after parsed expression");
...
console.log(`Extracted ${total} expected-success queries from conformance assertion helpers; skipped ${corpus.skippedInterpolated} interpolated templates.`);
console.log(`Grammar parser accepted ${accepted}/${total} extracted query inputs (${(accepted / total * 100).toFixed(1)}%).`);
for (const [kind, stat] of [...stats].sort((a, b) => b[1].seen - a[1].seen)) {
  console.log(`${kind.padEnd(12)} ${stat.accepted}/${stat.seen} parsed${stat.examples.length ? `; gaps: ${stat.examples.join(" | ")}` : ""}`);
}
