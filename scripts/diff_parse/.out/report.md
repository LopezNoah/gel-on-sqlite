...
> wrapper). The sharper question — _does it build the right node?_ — is below.
...
None. sqlite-ts's parser accepts every snippet Gel accepts (it errs toward leniency, hence the 5 over-accepts). The divergence is structural, not acceptance — see _Production faithfulness_ above.
