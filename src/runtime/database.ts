...
import {
  parseFixedOffsetHours,
  parseFormattedTemporal as parseTemporalFormat,
  parseLocalTemporal,
...
const compileRegex = (fname: string, source: string, flags: string): RegExp => {
  ...
  try {
    ...
    throw new AppError("E_RUNTIME", `invalid regular expression in ${fname}: '${source}'`, {
      cause,
    });
  }
};
...
const parseJsonArg = (fname: string, raw: string): unknown[] => {
  try {
    ...
    throw new AppError("E_RUNTIME", `${fname}: argument is not valid JSON: '${sample}'`, { cause });
  }
};
...
export const openSQLite = (target: string | Buffer = ":memory:"): SQLiteRuntime => {
  try {
    ...
    db.function("_gel_cot", (x: unknown) =>
      unaryTrig(x, "math::cot", (numeric) => 1 / Math.tan(numeric)),
    );
    ...
    db.function(
      "_gel_log",
      (x: number | null, base: number | null) =>
    ...
    db.function("_gel_assert", { varargs: true }, (...args: unknown[]) => {
      ...
      if (!truthy) {
        const msg =
          args.length > 1 && typeof args[1] === "string" && args[1] ? args[1] : "assertion failed";
    ...
    db.function("_gel_re_match_first", (pattern: string | null, value: string | null) => {
      ...
      if (!m) return null;
      const groups = (m.length === 1 ? [m[0]] : Array.from(m).slice(1)).map((group) => group ?? "");
    ...
    db.function("_gel_re_match_all", (pattern: string | null, value: string | null) => {
      ...
      while ((match = regex.exec(String(value))) !== null) {
        matches.push(
          (match.length === 1 ? [match[0]] : Array.from(match).slice(1)).map(
            (group) => group ?? "",
          ),
        );
    ...
    db.function("_gel_base64_decode", (value: string | null) => {
      ...
      if (invalid) {
        throw new AppError(
          "E_VALIDATION",
          `invalid symbol "${invalid[0]}" found while decoding base64 sequence`,
        );
      }
      if (
        value.length % 4 !== 0 ||
    ...
    db.function("_gel_assert_single", { varargs: true }, (...args: unknown[]) => {
      ...
      return arr.length === 0
        ? null
        : ((typeof arr[0] === "object" ? JSON.stringify(arr[0]) : arr[0]) as
            | number
            | string
            | null);
    });
    ...
    db.function("_gel_array_get", { varargs: true }, (...args: unknown[]) => {
      ...
      const arr =
        typeof a === "string" ? parseJsonArg("std::array_get", a) : Array.isArray(a) ? a : [];
    ...
    db.function(
      ...
      (
        ...
        const value =
          structuredValueRaw && typeof valueRaw === "string"
            ? parseJsonArg("std::find", valueRaw)
            : valueRaw;
        const start = Math.max(0, Number(startRaw ?? 0));
        const offset = values
          .slice(start)
          .findIndex((candidate) => isDeepStrictEqual(candidate, value));
        return offset < 0 ? -1 : start + offset;
      },
    );
    db.function(
      "_gel_array_contains",
      (arrayRaw: string | null, valueRaw: unknown, structuredValueRaw: number | null) => {
    ...
    db.function(
      "_gel_array_fill",
      (value: unknown, sizeRaw: number | null, unwrapTupleArrayRaw: number | null) => {
    ...
    db.function(
      "_gel_array_set",
      (a: string | null, idxRaw: number | null, valRaw: unknown, structuredRaw: number | null) => {
        ...
        const val =
          structuredRaw && typeof valRaw === "string"
    ...
    const statAggregate = (
      ...
      db.aggregate(name, {
        start: statStart,
        step: (state: StatState, value: unknown) => {
          statStep(state, value);
        },
    ...
    db.function("_gel_str_pad_start", { varargs: true }, (...a: unknown[]) =>
      padImpl(a[0], a[1], a[2], true),
    );
    db.function("_gel_str_pad_end", { varargs: true }, (...a: unknown[]) =>
      padImpl(a[0], a[1], a[2], false),
    );
    db.function("_gel_str_lower", (s: string | null) =>
      s === null ? null : String(s).toLowerCase(),
    );
    db.function("_gel_str_upper", (s: string | null) =>
      s === null ? null : String(s).toUpperCase(),
    );
    ...
    db.function("_gel_str_split", (s: string | null, delim: string | null) => {
      ...
      const parts = delim === "" ? [...str] : str === "" ? [] : str.split(String(delim));
    ...
    db.function("_gel_array_replace", { varargs: true }, (...a: unknown[]) => {
      ...
      const arr =
        typeof arrRaw === "string"
          ? parseJsonArg("std::array_replace", arrRaw)
          : (arrRaw as unknown[]);
      const oldValue =
        structuredRaw && typeof oldRaw === "string"
          ? parseJsonArg("std::array_replace", oldRaw)
          : oldRaw;
      const newValue =
        structuredRaw && typeof newRaw === "string"
          ? parseJsonArg("std::array_replace", newRaw)
          : newRaw;
      return JSON.stringify(
        arr.map((value) => (isDeepStrictEqual(value, oldValue) ? newValue : value)),
      );
    });
    ...
    const toNumberImpl = (
      ...
      if (fmt === "") {
        throw new AppError(
          "E_VALIDATION",
          `${fnName}(): "fmt" argument must be a non-empty string`,
        );
      }
      if (fmt === null || fmt === undefined) {
        ...
        if (s === "" || !Number.isFinite(num) || (integer && !Number.isInteger(num))) {
          throw new AppError(
            "E_VALIDATION",
            `invalid input syntax for type ${typeName}: '${rawStr}'`,
          );
        }
        return num;
      }
    ...
    db.function("_gel_to_int16", { varargs: true }, (...a: unknown[]) =>
      toNumberImpl(a[0], a[1], "to_int16", "std::int16", true),
    );
    db.function("_gel_to_int32", { varargs: true }, (...a: unknown[]) =>
      toNumberImpl(a[0], a[1], "to_int32", "std::int32", true),
    );
    db.function("_gel_to_int64", { varargs: true }, (...a: unknown[]) =>
      toNumberImpl(a[0], a[1], "to_int64", "std::int64", true),
    );
    db.function("_gel_to_float32", { varargs: true }, (...a: unknown[]) =>
      toNumberImpl(a[0], a[1], "to_float32", "std::float32", false),
    );
    db.function("_gel_to_float64", { varargs: true }, (...a: unknown[]) =>
      toNumberImpl(a[0], a[1], "to_float64", "std::float64", false),
    );
    db.function("_gel_to_bigint", { varargs: true }, (...a: unknown[]) =>
      toNumberImpl(a[0], a[1], "to_bigint", "std::bigint", true),
    );
    db.function("_gel_to_decimal", { varargs: true }, (...a: unknown[]) =>
      toNumberImpl(a[0], a[1], "to_decimal", "std::decimal", false),
    );
    ...
    db.function("_gel_range", { varargs: true }, (...a: unknown[]) => {
      ...
      const boolArg = (v: unknown, dflt: boolean): boolean =>
        v === null || v === undefined ? dflt : v === true || v === 1 || v === "true" || v === "1";
      ...
      if (discrete) {
        if (lower !== null && !incLower) {
          lower = stepBound(lower);
          incLower = true;
        }
        if (upper !== null && incUpper) {
          upper = stepBound(upper);
          incUpper = false;
        }
      }
      ...
      const empty =
        lower !== null && upper !== null && cmpBound(lower, upper) === 0 && !(incLower && incUpper);
      if (empty) {
    ...
    const lowerPos = (r: RangeObj): BoundPos =>
      r.lower === null
        ? { u: -1, v: null, eps: 0 }
        : { u: 0, v: r.lower, eps: r.inc_lower ? 0 : 1 };
    const upperPos = (r: RangeObj): BoundPos =>
      r.upper === null
        ? { u: 1, v: null, eps: 0 }
        : { u: 0, v: r.upper, eps: r.inc_upper ? 0 : -1 };
    ...
    const rangeContainsValue = (r: RangeObj, v: unknown): boolean => {
      ...
      const aboveLower =
        r.lower === null || (r.inc_lower ? cmpVals(x, r.lower) >= 0 : cmpVals(x, r.lower) > 0);
      const belowUpper =
        r.upper === null || (r.inc_upper ? cmpVals(x, r.upper) <= 0 : cmpVals(x, r.upper) < 0);
    ...
    const mkRange = (
      ...
      return {
        lower: lower as number | null,
        upper: upper as number | null,
        inc_lower: il,
        inc_upper: iu,
      };
    };
    const rawIsMultirange = (raw: unknown): boolean => {
      ...
      return (
        Array.isArray(o) || (Boolean(o) && Array.isArray((o as { ranges?: unknown[] }).ranges))
      );
    };
    ...
    const canonicalize = (list: RangeObj[]): RangeObj[] => asRangeList(JSON.stringify(list)) ?? [];
    ...
    const subtractOne = (a: RangeObj, b: RangeObj): RangeObj[] => {
      ...
      const out: RangeObj[] = [];
      if (cmpPos(lowerPos(a), lowerPos(b)) < 0)
        out.push(mkRange(a.lower, a.inc_lower, b.lower, !b.inc_lower));
      if (cmpPos(upperPos(a), upperPos(b)) > 0)
        out.push(mkRange(b.upper, !b.inc_upper, a.upper, a.inc_upper));
    ...
    const rangeSetOp = (
      ...
      const a = asRangeList(aRaw);
      const b = asRangeList(bRaw);
      if (!a || !b) return null;
      let result: RangeObj[];
      if (op === "union") {
    ...
    db.function("_gel_range_union", (aRaw: string | null, bRaw: string | null) =>
      rangeSetOp(aRaw, bRaw, "union"),
    );
    db.function("_gel_range_intersection", (aRaw: string | null, bRaw: string | null) =>
      rangeSetOp(aRaw, bRaw, "intersection"),
    );
    db.function("_gel_range_difference", (aRaw: string | null, bRaw: string | null) =>
      rangeSetOp(aRaw, bRaw, "difference"),
    );
    db.function("_gel_range_contains", (rRaw: string | null, v: unknown) => {
      ...
      if (
        ...
        return inner.every((b) =>
          rs.some(
            (a) => cmpPos(lowerPos(a), lowerPos(b)) <= 0 && cmpPos(upperPos(a), upperPos(b)) >= 0,
          ),
        )
          ? 1
          : 0;
    ...
    db.function("_gel_range_overlaps", (aRaw: string | null, bRaw: string | null) => {
      const r1s = asRangeList(aRaw);
      const r2s = asRangeList(bRaw);
      if (!r1s || !r2s) return null;
      return r1s.some((a) =>
        r2s.some(
          (b) => cmpPos(lowerPos(a), upperPos(b)) <= 0 && cmpPos(lowerPos(b), upperPos(a)) <= 0,
        ),
      )
        ? 1
        : 0;
    ...
    db.function("_gel_range_adjacent", (aRaw: string | null, bRaw: string | null) => {
      ...
      const meets = (x: RangeObj, y: RangeObj): boolean =>
        x.upper !== null &&
        y.lower !== null &&
        cmpVals(x.upper, y.lower) === 0 &&
        x.inc_upper !== y.inc_lower;
      const overlap = r1s.some((a) =>
        r2s.some(
          (b) => cmpPos(lowerPos(a), upperPos(b)) <= 0 && cmpPos(lowerPos(b), upperPos(a)) <= 0,
        ),
      );
      if (overlap) return 0;
      const lastA = r1s[r1s.length - 1];
      const firstA = r1s[0];
      const lastB = r2s[r2s.length - 1];
      const firstB = r2s[0];
    ...
    db.function("_gel_range_unpack", { varargs: true }, (...a: unknown[]) => {
      ...
      if (typeof r.lower === "string" || typeof r.upper === "string") {
        ...
        const parseTemporalStep = (v: unknown): TemporalStep => {
          ...
          if (m)
            return { ms: (Number(m[1]) * 3600 + Number(m[2]) * 60 + Number(m[3] ?? 0)) * 1000 };
          m = /^P(?:(\d+)M)?(?:(\d+)D)?$/.exec(s);
          ...
          if (m) {
            const mult = { day: 86400, hour: 3600, minute: 60, second: 1 }[
              m[2].toLowerCase() as "day" | "hour" | "minute" | "second"
            ];
        ...
        for (; r.inc_upper ? v <= hi : v < hi; v = advance(v)) {
          outS.push(fmt(v));
          if (outS.length > 1_000_000)
            throw new AppError("E_VALIDATION", "range_unpack result is too large");
        }
    ...
    db.function("_gel_float_cast", (v: unknown) => {
      ...
      if (s === "" || Number.isNaN(n)) {
        throw new AppError(
          "E_VALIDATION",
          `invalid input syntax for type std::float64: '${String(v)}'`,
        );
      }
      return n;
    });
    ...
    const parseIsoDuration = (s: string): DurParts | null => {
      ...
      return {
        ...
        us: Math.round(
          (Number(m[5] ?? 0) * 3600 + Number(m[6] ?? 0) * 60 + Number(m[7] ?? 0)) * 1e6,
        ),
      };
    };
    ...
    const parseFormattedTemporal = (
      ...
      if (!parsed.ok) {
        if (parsed.reason === "missingZoneFormat")
          throw new AppError("E_VALIDATION", "missing required time zone in format");
        if (parsed.reason === "unexpectedZoneFormat")
          throw new AppError("E_VALIDATION", "unexpected time zone in format");
        throw new AppError(
          "E_VALIDATION",
          requireZone
            ? "missing required time zone in input"
            : "invalid input syntax for temporal value",
        );
      }
      const { year, month, day, hour, minute, second } = parsed.parts;
      return {
        local: localDateTimeText(year, month, day, hour, minute, second),
        zone: parsed.zone,
      };
    };
    db.function("_gel_to_datetime", { varargs: true }, (...args: unknown[]) => {
      ...
      if (args.length === 2) {
        ...
        if (parsed.zone === null) {
          throw new AppError("E_VALIDATION", "missing required time zone in input");
        }
        return localInZoneToUtc(parsed.local, parsed.zone);
      }
      if (args.length === 7) {
        const local = localDateTimeText(
          ...(args.slice(0, 6).map(Number) as [number, number, number, number, number, number]),
        );
    ...
    db.function("_gel_to_local_datetime", { varargs: true }, (...args: unknown[]) => {
      ...
      if (args.length === 6)
        return localDateTimeText(
          ...(args.map(Number) as [number, number, number, number, number, number]),
        );
    ...
    db.function("_gel_to_local_date", { varargs: true }, (...args: unknown[]) => {
      ...
      if (args.length === 3)
        return localDateTimeText(Number(args[0]), Number(args[1]), Number(args[2]), 0, 0, 0).slice(
          0,
          10,
        );
    ...
    db.function("_gel_to_local_time", { varargs: true }, (...args: unknown[]) => {
      ...
      if (args.length === 2) {
        ...
        const second = String(args[1] ?? "");
        if (!second)
          throw new AppError("E_VALIDATION", '"fmt" argument must be a non-empty string');
    ...
    const dtUnitValue = (p: DtParts, unit: string, fname: string, allowEpoch: boolean): number => {
      switch (unit) {
        case "millennium":
          return Math.ceil(p.year / 1000);
        case "century":
          return Math.ceil(p.year / 100);
        case "decade":
          return Math.floor(p.year / 10);
        case "year":
          return p.year;
        case "quarter":
          return Math.floor((p.month - 1) / 3) + 1;
        case "month":
          return p.month;
        case "day":
          return p.day;
        ...
        case "seconds":
        case "second":
          return p.second + p.fracUs / 1e6;
        case "milliseconds":
          return p.second * 1000 + p.fracUs / 1000;
        case "microseconds":
          return p.second * 1e6 + p.fracUs;
        case "dow":
    ...
    db.function("_gel_time_get", (v: string | null, unit: string | null) => {
      ...
      if (
        !["hour", "minutes", "seconds", "milliseconds", "microseconds", "midnightseconds"].includes(
          u,
        )
      ) {
    ...
    db.function(
      "_gel_temporal_add_duration",
      (value: string | null, duration: string | null, signRaw: number | null) => {
        ...
        const date = new Date(
          Date.UTC(source.year, source.month - 1, 1, source.hour, source.minute, source.second),
        );
    ...
    db.function("_gel_duration_get", (dur: string | null, unit: string | null) => {
      ...
      switch (u) {
        case "millennium":
          return Math.trunc(p.months / 12000);
        case "century":
          return Math.trunc(p.months / 1200);
        case "decade":
          return Math.trunc(p.months / 120);
        case "year":
          return Math.trunc(p.months / 12);
        case "quarter":
          return Math.floor((((p.months % 12) + 12) % 12) / 3) + 1;
        case "month":
          return p.months % 12;
        case "day":
          return p.days;
        case "hour":
          return Math.trunc(p.us / 3.6e9);
        case "minutes":
          return Math.trunc((p.us % 3.6e9) / 6e7);
        case "seconds":
          return (p.us % 6e7) / 1e6;
        case "milliseconds":
          return (p.us % 6e7) / 1000;
        case "microseconds":
          return p.us % 6e7;
        case "totalseconds":
          return (p.months * 2592000 * 1e6 + p.days * 86400 * 1e6 + p.us) / 1e6;
        default:
          return invalidUnit("std::duration_get", u);
      }
    });
    // `std::duration_truncate(dur, unit)` — plural unit names.
    db.function(
      "_gel_duration_truncate",
      { varargs: true },
      (dur: string | null, unit: string | null, typeName?: string | null) => {
        if (dur === null || unit === null) return null;
        const p = parseIsoDuration(String(dur));
        if (!p) throw new AppError("E_VALIDATION", `invalid duration value: '${dur}'`);
        const u = String(unit).toLowerCase();
        if (
          typeName?.includes("date_duration") &&
          ["hours", "minutes", "seconds", "milliseconds", "microseconds"].includes(u)
        ) {
        ...
        switch (u) {
          case "millenniums":
          case "millennia":
            p.months = Math.trunc(p.months / 12000) * 12000;
            p.days = 0;
            p.us = 0;
            break;
          case "centuries":
            p.months = Math.trunc(p.months / 1200) * 1200;
            p.days = 0;
            p.us = 0;
            break;
          case "decades":
            p.months = Math.trunc(p.months / 120) * 120;
            p.days = 0;
            p.us = 0;
            break;
          case "years":
            p.months = Math.trunc(p.months / 12) * 12;
            p.days = 0;
            p.us = 0;
            break;
          case "quarters":
            p.months = Math.trunc(p.months / 3) * 3;
            p.days = 0;
            p.us = 0;
            break;
          case "months":
          ...
          case "days":
            ...
            break;
          case "hours":
            p.us = Math.trunc(p.us / 3.6e9) * 3.6e9;
            break;
          case "minutes":
            p.us = Math.trunc(p.us / 6e7) * 6e7;
            break;
          case "seconds":
            p.us = Math.trunc(p.us / 1e6) * 1e6;
            break;
          case "milliseconds":
            p.us = Math.trunc(p.us / 1000) * 1000;
            break;
          // `p.us` is already microsecond-precision, so truncating to µs is a no-op.
          case "microseconds":
            break;
          default:
            return invalidUnit("std::duration_truncate", u);
        }
        return formatDur(p);
      },
    );
    ...
    db.function("_gel_datetime_truncate", (v: string | null, unit: string | null) => {
      ...
      switch (u) {
        case "millenniums":
        case "millennia":
          p.year = Math.floor((p.year - 1) / 1000) * 1000 + 1;
          p.month = 1;
          p.day = 1;
          p.hour = 0;
          p.minute = 0;
          ...
          break;
        case "centuries":
          p.year = Math.floor((p.year - 1) / 100) * 100 + 1;
          p.month = 1;
          p.day = 1;
          p.hour = 0;
          p.minute = 0;
          ...
          break;
        case "decades":
          p.year = Math.floor(p.year / 10) * 10;
          p.month = 1;
          p.day = 1;
          p.hour = 0;
          p.minute = 0;
        ...
        case "years":
          ...
          break;
        case "quarters":
          p.month = Math.floor((p.month - 1) / 3) * 3 + 1;
          p.day = 1;
          p.hour = 0;
          p.minute = 0;
          p.second = 0;
        ...
        case "seconds":
          ...
          break;
        default:
          return invalidUnit("std::datetime_truncate", u);
      }
