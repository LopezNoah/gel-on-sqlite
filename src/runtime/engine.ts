...
// The deps the session-globals lifecycle (runtime/globals.ts) injects. A
// lazily-evaluated factory: `normalizeSecurityContext` is a const arrow
// declared far below, so a top-level const object would hit its TDZ; the
// factory only reads at call time (same pattern as selectExprEvaluatorDeps).
const globalsDeps = (): GlobalsDeps => ({
  tryRunSingleSqlRows,
  normalizeSecurityContext,
  DEFAULT_SECURITY_CONTEXT,
});

// The deps function dispatch (runtime/function_dispatch.ts) injects: running a
// UDF's SELECT body (the engine entry point, declared below) and the runtime
// set-cardinality counter. Lazy for the same TDZ reason as globalsDeps.
const fnDispatchDeps = (): FunctionDispatchDeps => ({
  executeQuery,
  ...
  try {
    ...
    if (ast.kind === "configure") {
      ...
      if (ast.isSessionGlobal) {
        applySessionGlobal(db, schema, ast, context, globalsDeps());
        return {
          ast,
          ir: undefined,
...
//
// The cluster lives in ./ast_validation.ts behind `validateStatementAst` (its
// own test surface). Here we only supply the deps it reaches back into — the
// engine's runtime-alias registries — and keep the original
// `preValidateStatementAst(schema, statement, allowUserSpecifiedId)` call shape.
// ═══════════════════════════════════════════════════════════════════════════

const astValidationDeps: AstValidationDeps = {
  runtimeTypedAliasMap: getRuntimeTypedAliasMap,
