...
export const compileToIR = (schema: SchemaSnapshot, statement: Statement, context: CompileContext = {}): IRStatement => {
  ...
  // INSERT/UPDATE/DELETE compile through the standalone DML module — the
  // runtime reaches it directly via the compiler service and never enters
  // this function for mutations. This delegation keeps direct compileToIR
  // callers (the legacy IR inference tests, debug dumps) working, layering
  // on the WITH-binding multiplicity refinement that needs the legacy
  // inference fold (`X := {User, User} UPDATE X …` stays "many").
  if (statement.kind === "insert" || statement.kind === "update" || statement.kind === "delete") {
    const plan = compileDmlToIR(schema, statement, { globals: context.globals });
    if (plan.kind === "update") {
      for (const binding of statement.with ?? []) {
        if (binding.value.kind === "subquery_expr") {
          bindingMults.set(binding.name, inferAstMultiplicity(binding.value.expr));
        }
      }
      if (bindingMults.get(statement.typeName) === "duplicate" && plan.inference) {
        plan.inference = { ...plan.inference, cardinality: "many" };
      }
    }
    return plan;
  }
  ...
  return fail(`Statement kind '${statement.kind}' is not supported by the legacy IR compiler`);
};
