  ...
  expr.kind === "binding_ref"
    ...
    : expr.kind === "path" && bindings.has(expr.head)
      ? {
          ...
          expr: {
            kind: "field_access",
            expr: { kind: "binding_ref", name: expr.head },
            field: expr.tail,
            optional: false,
          },
        }
      : expr.kind === "select" &&
          expr.shape.length === 1 &&
          expr.shape[0].kind === "field" &&
          expr.shape[0].name === "id" &&
          Object.keys(expr.clauses).length === 0
        ? { kind: "binding_ref", name: expr.typeName }
        : { kind: "expr", expr };
const validateFunctionCallArgs = (
  args: readonly FunctionCallArgExpr[],
  line: number,
  column: number,
): void => {
  let sawNamed = false;
...
const insertValue = (expr: FreeObjectExpr): InsertValue => {
  ...
  if (
    ...
    expr.values.every((value) => value.kind === "literal")
  ) {
    return {
      kind: "array_literal",
      values: expr.values.map(
        (value) => (value as Extract<FreeObjectExpr, { kind: "literal" }>).value,
      ),
    };
  }
...
const filterFromExpr = (predicate: FreeObjectExpr): FilterExpr => {
  ...
  if (predicate.kind === "in_expr") {
    ...
    const values =
      predicate.right.kind === "set_literal"
        ? predicate.right
        : predicate.right.kind === "set_expr"
          ? { kind: "expr_set" as const, values: predicate.right.values }
          : predicate.right.kind === "select"
            ? {
                kind: "select" as const,
                query: {
                  typeName: predicate.right.typeName,
                  shape: predicate.right.shape,
                  clauses: predicate.right.clauses,
                },
              }
...
function insertConflictAlternative(
  ...
  if (alternative.kind === "type_name") {
    return {
      kind: "select",
      typeName: inModule(alternative.name, defaultModule),
      shape: defaultShape(),
      clauses: {},
    };
  }
  ...
  const query =
    alternative.expr.kind === "select_expr_subquery" ? alternative.expr.expr : alternative.expr;
...
function numberLiteral(text: string, line: number, column: number): FreeObjectExpr {
  ...
  const numericKind = cleaned.endsWith("n")
    ? /[.eE]/.test(cleaned)
      ? "decimal"
      : "bigint"
    : /[.eE]/.test(cleaned)
...
function orderExprForSelect(item: GrammarOrderItem, typeName: string): OrderExpr {
  ...
  const nameSort =
    item.expr.kind === "binding_ref"
      ? item.expr.name
      : item.expr.kind === "path"
        ? `${item.expr.head}.${item.expr.tail}`
        : undefined;
  return {
    field: orderField
      ? (item.expr as Extract<FreeObjectExpr, { kind: "field_access" }>).field
      : (nameSort ?? "__expr__"),
...
function pathStepsForExpr(expr: FreeObjectExpr): PathStep[] | undefined {
  ...
  if (expr.kind === "path")
    return expr.steps
      ? [...expr.steps]
      : [
          { kind: "object_ref", name: expr.head },
          { kind: "ptr", name: expr.tail, direction: "outbound" },
        ];
  if (expr.kind === "path_chain") {
    const [head, ...tail] = expr.parts;
    return head
      ? [
          { kind: "object_ref", name: head },
          ...tail.map((name) => ({ kind: "ptr" as const, name, direction: "outbound" as const })),
        ]
      : undefined;
  }
  ...
  if (expr.kind === "field_access") {
    ...
    return base
      ? [...base, { kind: "ptr", name: expr.field, direction: "outbound", optional: expr.optional }]
      : undefined;
  }
...
function mutationTargetTypeName(expr: FreeObjectExpr, bindings: ReadonlySet<string>): string {
  switch (expr.kind) {
    ...
    case "path":
      return expr.head;
    case "path_chain":
      return expr.parts[0] ?? "Object";
    case "path_steps": {
      const first = expr.steps[0];
      return first?.kind === "object_ref" ? first.name : "Object";
    }
    case "set_expr":
      return expr.values[0] ? mutationTargetTypeName(expr.values[0], bindings) : "Object";
    ...
    case "field_access":
    case "is_type":
      return mutationTargetTypeName(expr.expr, bindings);
    case "binding_ref":
      return bindings.has(expr.name) ? "Object" : expr.name;
    default:
      return "Object";
  }
}
...
class GrammarParser extends EmbeddedActionsParser {
  ...
  constructor() {
    ...
    // Chevrotain's DSL convention uses `$` as the grammar receiver.
    ...
    $.RULE("statement", (): Statement => {
      ...
      $.OPTION(() => {
        ...
        $.MANY({
          GATE: () =>
            tokenMatcher($.LA(1), tokens.comma) && !tokenMatcher($.LA(2), tokens.kw_select),
          DEF: () => {
        ...
        $.OPTION6({
          GATE: () =>
            tokenMatcher($.LA(1), tokens.comma) && tokenMatcher($.LA(2), tokens.kw_select),
          DEF: () => $.CONSUME3(tokens.comma),
        });
      });
      const start = $.CONSUME(tokens.kw_select);
      let resultAlias: string | undefined;
      $.OPTION7({
        GATE: () => tokenMatcher($.LA(1), Name) && tokenMatcher($.LA(2), tokens.assign),
        DEF: () => {
          resultAlias = $.CONSUME(Name).image;
          $.CONSUME(tokens.assign);
          $.ACTION(() => {
            if (resultAlias !== undefined) $.bindings.add(resultAlias);
          });
        },
      });
      const head = $.LA(1);
      ...
      return $.ACTION(() => {
        const pos = { line: start.startLine ?? 1, column: start.startColumn ?? 1 };
        const shapeProjection = result.kind === "shape_projection" ? result : undefined;
        const shapePath =
          shapeProjection?.expr.kind === "path_steps" ? shapeProjection.expr : undefined;
        const rootStep = shapePath?.steps[0];
        const filterSteps = shapePath?.steps.slice(1);
        const typedObjectShape =
          shapeProjection &&
          rootStep?.kind === "object_ref" &&
          filterSteps?.length &&
          filterSteps.every((step) => step.kind === "type_intersection" && step.typeExpr)
            ? {
                subject: {
                  kind: "select" as const,
                  typeName: rootStep.name,
                  shape: shapeProjection.shape,
                  clauses: {},
                },
                filters: filterSteps.map((step) => {
                  if (step.kind !== "type_intersection" || !step.typeExpr) {
                    throw new Error("Expected a type-intersection filter");
                  }
                  return step.typeExpr;
                }),
              }
            : undefined;
        const baseResult = typedObjectShape?.subject ?? result;
        const subject =
          baseResult.kind === "binding_ref" &&
          !$.bindings.has(baseResult.name) &&
          (filter ||
            filterExpr ||
            order ||
            limit !== undefined ||
            ...
            limitExpr ||
            offsetExpr)
            ? typeSource(inModule(baseResult.name, $.defaultModule))
            : baseResult;
        ...
        if (
          subject.kind === "free_object_constructor" &&
          head.tokenTypeIdx === tokens.lbrace.tokenTypeIdx
        ) {
          if (order || offset !== undefined || limit !== undefined || offsetExpr || limitExpr) {
            throw syntaxError(
              "Free object clauses are outside the grammar-backed AST slice",
              pos.line,
              pos.column,
            );
          }
        ...
        const expr: FreeObjectExpr =
          offset !== undefined || limit !== undefined || offsetExpr || limitExpr
            ? {
                kind: "select_expr_subquery",
                expr: subject,
                ...(offset !== undefined ? { offset } : {}),
    ...
    $.RULE("insertStatement", (): Statement => {
        ...
        $.OPTION5(() => {
          ...
          const alternative = $.OR2([
            ...
            },
            {
              ...
              ALT: () => ({
                kind: "update_statement" as const,
                statement: $.SUBRULE($.updateStatement),
              }),
            },
            {
              GATE: () => tokenMatcher($.LA(1), tokens.kw_select),
              ALT: () => ({
                kind: "select_expr" as const,
                expr: $.SUBRULE($.innerSelect),
              }),
            },
      ...
      return $.ACTION(() => ({
        kind: "insert",
        typeName: inModule(typeName, $.defaultModule),
        values,
        ...(conflict
          ? {
              conflict: {
                ...(onFields?.length
                  ? { onField: onFields[0], ...(onFields.length > 1 ? { onFields } : {}) }
                  : {}),
    ...
    $.RULE("deleteStatement", (): Statement => {
      ...
      return $.ACTION(() => {
        ...
        if (typeof subject === "string") {
          return {
            kind: "delete",
            typeName: inModule(subject, $.defaultModule),
            ...(filter ? { filter } : {}),
            pos,
          };
        }
        if (subject.kind !== "select_expr_subquery" || subject.expr.kind !== "select" || filter) {
          throw syntaxError(
            "DELETE target is outside the grammar-backed AST slice",
            pos.line,
            pos.column,
          );
        }
    ...
    $.RULE("updateSubject", (): { typeName: string; target?: FreeObjectExpr } => {
      const isExpressionTarget = (): boolean => {
        ...
        if (
          [
            ...
            tokens.lparen,
            tokens.dot,
            tokens.backward_link,
            tokens.optional_link,
          ].some((kind) => tokenMatcher(first, kind))
        )
          return true;
        return (
          tokenMatcher(first, Name) &&
          ([
            tokens.lbracket,
            tokens.lbrace,
            tokens.dot,
      ...
      return $.OR([
        {
          GATE: isExpressionTarget,
          ALT: () => {
            const target = $.SUBRULE($.expression);
            return $.ACTION(() => ({
              typeName: mutationTargetTypeName(target, $.bindings),
              target,
            }));
          },
        },
    ...
    $.RULE("updateStatement", (): Statement => {
      ...
      return $.ACTION(() => ({
        kind: "update",
        typeName: inModule(subject.typeName, $.defaultModule),
        ...(subject.target ? { target: subject.target } : {}),
        ...(filter ? { filter } : {}),
        values,
        operations,
        pos: { line: start.startLine ?? 1, column: start.startColumn ?? 1 },
      ...
      (): {
        ...
        $.ACTION(() => {
          ...
          if (iteratorExpr.kind === "select") {
            $.bindingValues.set(variable, {
              kind: "subquery",
              query: {
                typeName: iteratorExpr.typeName,
                shape: iteratorExpr.shape,
                clauses: {},
              },
            });
          }
    ...
    $.RULE("forStatement", (): Statement => {
      ...
      const body = $.OR([
            ...
            return $.ACTION(() => ({
              ...
              pos:
                ...
                inner.tokenTypeIdx === tokens.kw_select.tokenTypeIdx
                  ? { line: inner.startLine ?? 1, column: inner.startColumn ?? 1 }
                  : start.pos,
            }));
          },
        },
        {
          GATE: () =>
            [tokens.kw_insert, tokens.kw_update, tokens.kw_delete].some((kind) =>
              tokenMatcher($.LA(1), kind),
            ),
          ALT: () =>
            $.OR2([
      ...
      return $.ACTION(() => {
        ...
        for (let i = binders.length - 1; i >= 0; i--) {
          ...
          nested = {
            ...
            iteratorExpr: binder.iteratorExpr,
            body: nested as ForStatement["body"],
            pos: binder.pos,
          };
        }
        return nested;
    ...
    $.RULE("groupStatement", (): GroupStatement => {
      ...
      $.OPTION(() => {
        ...
        $.MANY2({
          GATE: () =>
            tokenMatcher($.LA(1), tokens.comma) && !tokenMatcher($.LA(2), tokens.kw_group),
          DEF: () => {
            $.CONSUME(tokens.comma);
            withBindings.push($.SUBRULE2($.binding));
          },
        ...
        $.OPTION2({
          GATE: () => tokenMatcher($.LA(1), tokens.comma) && tokenMatcher($.LA(2), tokens.kw_group),
          DEF: () => $.CONSUME3(tokens.comma),
        });
      });
      const start = $.LA(1);
    ...
    $.RULE("groupExpr", (): GroupExpr => {
      ...
      if (source.kind === "select") {
        source = {
          ...source,
          shape: source.shape.map((element) => ({
            ...element,
            ...
            limit: element.limit,
            offsetExpr: element.offsetExpr,
            limitExpr: element.limitExpr,
          })),
        };
      }
      ...
      $.OPTION(() => {
        ...
        $.MANY2({
          GATE: () => tokenMatcher($.LA(1), tokens.comma) && !tokenMatcher($.LA(2), tokens.kw_by),
          DEF: () => {
            $.CONSUME(tokens.comma);
      ...
      return $.ACTION(() => ({
        kind: "group_expr",
        source,
        using: using.length ? using : undefined,
        by,
      }));
    });
    ...
    $.RULE("groupByElement", (): GroupByElement => {
      const result = $.OR([
        ...
        {
          GATE: () =>
            tokenMatcher($.LA(1), Name) &&
            tokenMatcher($.LA(2), tokens.lparen) &&
            ["cube", "rollup"].includes($.LA(1).image.toLowerCase()),
          ALT: () => {
            const operation = $.CONSUME(Name).image.toLowerCase();
            $.CONSUME(tokens.lparen);
            const atoms = $.SUBRULE4($.groupByAtomList);
            $.CONSUME(tokens.rparen);
            return operation === "cube"
              ? { kind: "cube" as const, atoms }
              : { kind: "rollup" as const, atoms };
          },
        },
        {
          ALT: () => {
    ...
    $.RULE("binding", (): WithBinding => {
      ...
      return $.ACTION(() => {
        $.bindings.add(name);
        const mapped: WithBinding["value"] =
          value.kind === "select" &&
          (head.image === value.typeName.split("::")[0] ||
            head.tokenTypeIdx === tokens.kw_detached.tokenTypeIdx)
            ? ({
                kind: "subquery",
                query: {
                  kind: "select" as const,
    ...
    $.RULE("withDmlStatement", (): Statement => {
      ...
      const finalStatement = () =>
        [tokens.kw_insert, tokens.kw_update, tokens.kw_delete, tokens.kw_for].some((kind) =>
          tokenMatcher($.LA(2), kind),
        );
      $.MANY({
        GATE: () => tokenMatcher($.LA(1), tokens.comma) && !finalStatement(),
        DEF: () => {
          $.CONSUME(tokens.comma);
          withBindings.push($.SUBRULE2($.binding));
        },
    ...
    $.RULE("orderSuffix", (): GrammarOrderSuffix => {
      ...
      $.OPTION(() =>
        $.OR([
          {
            ALT: () => {
              $.CONSUME(tokens.kw_asc);
              direction = "asc";
            },
          },
          {
            ALT: () => {
              $.CONSUME(tokens.kw_desc);
              direction = "desc";
            },
          },
        ]),
      ...
      $.OPTION2(() => {
        ...
        nullsPosition = $.ACTION(() => {
          ...
          if (lowered !== "first" && lowered !== "last") {
            throw syntaxError(
              "Expected FIRST or LAST after EMPTY",
    ...
    $.RULE("expression", (): FreeObjectExpr => {
      ...
      $.MANY(() => {
        ...
        result = $.ACTION(() =>
          op.tokenTypeIdx === tokens.kw_union.tokenTypeIdx
            ? result.kind === "set_expr"
              ? { kind: "set_expr", values: [...result.values, right] }
              : { kind: "set_expr", values: [result, right] }
            : {
                kind: "set_op",
                op: op.tokenTypeIdx === tokens.kw_intersect.tokenTypeIdx ? "intersect" : "except",
                left: result,
                right,
              },
        );
    ...
    $.RULE("comparison", (): FreeObjectExpr => {
      ...
      $.OPTION5({
        GATE: () =>
          tokenMatcher($.LA(1), tokens.kw_not) &&
          [tokens.kw_like, tokens.kw_ilike].some((kind) => tokenMatcher($.LA(2), kind)),
        DEF: () => {
          ...
          result = $.ACTION(() => ({
            kind: "compare",
            op: `not_${like.image}` as CompareOp,
            left,
            right,
          }));
        },
    ...
    $.RULE("additive", (): FreeObjectExpr => {
      ...
      $.MANY(() => {
        ...
        left = $.ACTION(() =>
          op.image === "++"
            ? left.kind === "concat"
              ? { kind: "concat", parts: [...left.parts, right] }
              : { kind: "concat", parts: [left, right] }
      ...
      $.OR([
        ...
        },
        {
          ALT: () => {
            ...
            let optional = false;
            $.OPTION3({
              GATE: () => tokenMatcher($.LA(1), tokens.kw_optional),
              DEF: () => {
                $.CONSUME(tokens.kw_optional);
                optional = true;
              },
            ...
            $.OPTION4({
              GATE: () => tokenMatcher($.LA(1), tokens.kw_required),
              DEF: () => {
        ...
        {
          GATE: () => tokenMatcher($.LA(1), tokens.kw_introspect),
          ALT: () => {
            $.CONSUME(tokens.kw_introspect);
            let typeofForm = false;
            $.OPTION({
              GATE: () => tokenMatcher($.LA(1), tokens.kw_typeof),
              DEF: () => {
            ...
            const expr = $.OR2([
              {
                GATE: () =>
                  !typeofForm &&
                  tokenMatcher($.LA(1), tokens.lparen) &&
                  tokenMatcher($.LA(2), Name) &&
                  tokenMatcher($.LA(3), tokens.lt),
                ALT: () => {
                  $.CONSUME(tokens.lparen);
        ...
        $.OR([
          {
            ALT: () => {
              ...
              expr = $.ACTION(() => ({
                kind: "for_expr",
                variable: "__gel_backlink_item__",
                iterator: expr,
                body: {
                  kind: "backlink_path",
                  link,
                  sourceType: simpleTypeName(sourceType),
                  sourceTypeExpr: sourceType,
                  optional: false,
                },
              }));
              void backlink;
          ...
          },
          {
            ALT: () => {
              ...
              const member = $.OR3([
                {
                  GATE: () => tokenMatcher($.LA(1), tokens.number),
                  ALT: () => ({ index: $.CONSUME(tokens.number) }),
                },
                ...
                return expr.kind === "path_steps"
                  ...
                  : expr.kind === "binding_ref" &&
                    ...
                    ? {
                        ...
                        tail: field,
                        steps: [
                          { kind: "object_ref" as const, name: expr.name },
                          { kind: "ptr" as const, name: field, direction: "outbound" as const },
                        ],
                      }
                    : {
                        kind: "field_access" as const,
                        expr:
                          expr.kind === "field_access" && expr.expr.kind === "current_item"
                            ? {
                                kind: "field_access" as const,
                                expr: expr.expr,
                                field: expr.field,
          ...
          },
          {
            ALT: () => {
              ...
              const subscript = $.OR2([
                {
                  GATE: () => tokenMatcher($.LA(1), tokens.kw_is),
                  ALT: () => {
                ...
                },
                {
                  ALT: () => {
                    ...
                    $.OPTION2({
                      GATE: () =>
                        !tokenMatcher($.LA(1), tokens.colon) &&
                        !tokenMatcher($.LA(1), tokens.rbracket),
                      DEF: () => {
                        first = $.SUBRULE($.subscriptBound);
                      },
                    });
                    let isSlice = false;
                    $.OPTION3(() => {
                      ...
                      isSlice = true;
                      $.OPTION4({
                        GATE: () => !tokenMatcher($.LA(1), tokens.rbracket),
                        DEF: () => {
                    ...
                    return $.ACTION<FreeObjectExpr>(() => {
                      if (isSlice)
                        return {
                          kind: "slice_access",
                          expr,
                          start: numberValue(first),
                          end: numberValue(end),
                          ...(first && numberValue(first) === undefined
                            ? { startExpr: first }
                            : {}),
    ...
    $.RULE("castTypeArg", (): string =>
      $.OR([
        {
          GATE: () => tokenMatcher($.LA(1), Name) && tokenMatcher($.LA(2), tokens.colon),
          ALT: () => {
            const name = $.CONSUME(Name).image;
            $.CONSUME(tokens.colon);
    ...
    $.RULE("shapeEntry", (): ShapeElement => {
      ...
      const isModifier = (): boolean => {
        const token = $.LA(1);
        const terminator = [tokens.comma, tokens.rbrace, tokens.colon, tokens.assign].some((kind) =>
          tokenMatcher($.LA(2), kind),
        );
        return (
          !terminator &&
          [tokens.kw_required, tokens.kw_optional, tokens.kw_multi, tokens.kw_single].some((kind) =>
            tokenMatcher(token, kind),
          )
        );
      };
      $.MANY({
      ...
      $.OR([
        ...
        },
        {
          GATE: () =>
            ...
            tokenMatcher($.LA(3), tokens.backward_link),
          ALT: () => $.SUBRULE($.computedBacklinkShapeEntry),
        },
        {
          ALT: () => {
    ...
    $.RULE("atom", (): FreeObjectExpr =>
      $.OR([
        {
          GATE: () => tokenMatcher($.LA(1), tokens.str_interp_start),
          ALT: () => $.SUBRULE($.stringInterpolation),
        },
        {
          GATE: () => tokenMatcher($.LA(1), tokens.lbracket) && tokenMatcher($.LA(2), tokens.kw_is),
          ALT: () => {
            ...
            $.CONSUME(tokens.rbracket);
            return {
              kind: "path_steps" as const,
              steps: [
                {
                  kind: "type_intersection" as const,
                  typeName: simpleTypeName(typeExpr) ?? "",
                  typeExpr,
                },
              ],
        ...
        },
        {
          ALT: () => {
            ...
            return $.ACTION(() =>
              numberLiteral(literal.image, literal.startLine ?? 1, literal.startColumn ?? 1),
            );
          },
        },
        {
        ...
        {
          ALT: () => {
            $.CONSUME(tokens.kw_true);
            return $.ACTION<FreeObjectExpr>(() => ({ kind: "literal", value: true }));
          },
        },
        {
          ALT: () => {
            $.CONSUME(tokens.kw_false);
            return $.ACTION<FreeObjectExpr>(() => ({ kind: "literal", value: false }));
          },
        },
        {
          ALT: () => {
            $.CONSUME(tokens.kw_null);
            return $.ACTION<FreeObjectExpr>(() => ({ kind: "literal", value: null }));
          },
        },
        {
          ALT: () => {
            ...
            return $.ACTION<FreeObjectExpr>(() => {
              if (
                expr.kind === "select" ||
                expr.kind === "select_expr_subquery" ||
                expr.kind === "free_object_constructor"
              )
                return { ...expr, detached: true };
              return expr;
            });
          },
        },
        ...
        },
        {
          ALT: () => {
            ...
            const member = $.OR2([
              {
                GATE: () => tokenMatcher($.LA(1), tokens.number),
                ALT: () => ({ index: $.CONSUME2(tokens.number) }),
              },
          ...
          ALT: () => {
            ...
            return $.ACTION<FreeObjectExpr>(() => {
              if (name.startsWith("__") && name.endsWith("__")) {
                if (name === "__type__")
                  throw syntaxError(
                    "'__type__' is always a path step; reference it as 'X.__type__'",
                  );
                return { kind: "global_ref", name };
              }
              return (
                result ??
                ($.bindings.has(name) || !/[A-Z]/.test(name.split("::").pop()?.[0] ?? "")
                  ? { kind: "binding_ref", name }
                  : typeSource(inModule(name, $.defaultModule)))
              );
            });
          },
        },
    ...
    $.RULE("functionCallArg", (): FunctionCallArgExpr =>
      $.OR([
        {
          GATE: () =>
            (tokenMatcher($.LA(1), Name) || tokenMatcher($.LA(1), tokens.kw_empty)) &&
            tokenMatcher($.LA(2), tokens.assign),
          ALT: () => {
            const name = $.OR2([
            ...
            $.CONSUME(tokens.assign);
            const expr = $.SUBRULE($.expression);
            return $.ACTION<FunctionCallArgExpr>(() => ({
              kind: "named_arg",
              name,
              arg: functionArg(expr, $.bindings),
            }));
          },
        },
        {
    ...
    $.RULE("innerSelect", (): FreeObjectExpr => {
      ...
      $.OPTION4({
        ...
        DEF: () => {
          ...
          $.CONSUME(tokens.assign);
          $.ACTION(() => {
            if (alias !== undefined) $.bindings.add(alias);
          });
        },
      });
      const expr = $.SUBRULE($.expression);
    ...
    $.RULE(
      ...
      (): {
        ...
        const isModifier = (): boolean => {
          ...
          return (
            !terminator &&
            [tokens.kw_required, tokens.kw_optional, tokens.kw_multi, tokens.kw_single].some(
              (kind) => tokenMatcher($.LA(1), kind),
            )
          );
        };
        $.MANY({
    ...
    $.RULE("parenthesized", (): FreeObjectExpr => {
      ...
      const result = $.OR([
        {
          GATE: () => tokenMatcher($.LA(1), tokens.rparen),
          ALT: () => {
            $.CONSUME(tokens.rparen);
        ...
        },
        {
          ...
          ALT: () => {
            ...
            const expr = $.OR2([
              {
                GATE: () => tokenMatcher($.LA(1), tokens.kw_select),
                ALT: () => $.SUBRULE2($.innerSelect),
              },
              {
                GATE: () => tokenMatcher($.LA(1), tokens.kw_group),
                ALT: () => $.SUBRULE($.groupExpr),
              },
              {
                GATE: () => tokenMatcher($.LA(1), tokens.kw_for),
                ALT: () => $.SUBRULE2($.forExpr),
              },
            ]);
    ...
    $.RULE("stringInterpolation", (): FreeObjectExpr => {
      ...
      $.MANY({
        GATE: () => tokenMatcher($.LA(1), tokens.str_interp_cont),
        DEF: () => {
    ...
    $.RULE("braces", (): FreeObjectExpr => {
      ...
      const isObjectEntryAhead = (): boolean => {
        ...
        while (
          [tokens.kw_required, tokens.kw_optional, tokens.kw_multi, tokens.kw_single].some((kind) =>
            tokenMatcher($.LA(offset), kind),
          ) &&
          ![tokens.comma, tokens.rbrace, tokens.colon, tokens.assign].some((kind) =>
            tokenMatcher($.LA(offset + 1), kind),
          )
        )
          offset++;
      ...
      const result = $.OR([
        ...
        },
        {
          ALT: () => {
            ...
            return $.ACTION<FreeObjectExpr>(() =>
              values.every(
                (v) => v.kind === "literal" && (!v.numericKind || v.numericKind === "integer"),
              )
                ? {
                    kind: "set_literal",
                    values: values.map(
                      (v) => (v as Extract<FreeObjectExpr, { kind: "literal" }>).value,
                    ),
                  }
...
const isWordToken = (token: Token | undefined): boolean =>
  !!token &&
  ![
    "eof",
    "semi",
...
function parseFunctionDecl(
  ...
  const parseParam = (from: number, to: number): void => {
    ...
    let optional = false;
    let setOf = false;
    if (tokensIn[typeStart]?.lower === "optional") {
      optional = true;
      typeStart++;
    }
    ...
    const type =
      typeEnd > typeStart
        ? source
            .slice(
              tokensIn[typeStart].offset,
              tokensIn[typeEnd - 1].offset + tokensIn[typeEnd - 1].lexeme.length,
            )
            .trim()
        : "";
    ...
    if (typeEnd < to) {
      ...
      if (defaultStart < to)
        defaultExpr = source
          .slice(
            tokensIn[defaultStart].offset,
            tokensIn[to - 1].offset + tokensIn[to - 1].lexeme.length,
          )
          .trim();
    }
    params.push({
  ...
  if (tokensIn[arrow]?.lower === "optional") {
    returnOptional = true;
    arrow++;
  }
  ...
  let using = arrow;
  while (
    using < tokensIn.length &&
    tokensIn[using].kind !== "kw_using" &&
    tokensIn[using].kind !== "lbrace"
  )
    using++;
  ...
  const returnType =
    returnEnd > arrow
      ? source
          .slice(
            tokensIn[arrow].offset,
            tokensIn[returnEnd - 1].offset + tokensIn[returnEnd - 1].lexeme.length,
          )
          .trim()
      : "";
  if (tokensIn[using]?.kind === "lbrace") {
    const innerUsing = tokensIn.findIndex(
      (token, index) => index > using && token.kind === "kw_using",
    );
...
function functionSetCommands(tokensIn: readonly Token[]): string[] {
  ...
  for (let i = 0; i + 1 < tokensIn.length; i++) {
    if (tokensIn[i].lower === "set" && isWordToken(tokensIn[i + 1]))
      commands.push(tokensIn[i + 1].lexeme);
  }
  return commands;
}

/** Parse the shared DDL header and the type-body forms modelled by the AST. */
function parseDdlGrammar(
  source: string,
  ...
  const action: DDLStatement["action"] =
    start.kind === "kw_create" ? "create" : start.kind === "kw_alter" ? "alter" : "drop";
  const modifiers: string[] = [];
  let i = 1;
  while (i < input.length && !ddlKinds[input[i].lower]) {
    if (!isWordToken(input[i])) {
      throw syntaxError(
        `Expected DDL object kind, found '${input[i].lexeme}'`,
        input[i].offset < 0 ? 1 : offsetToLineCol(input[i].offset, lineStarts).line,
  ...
  if (action === "create" && (objectKind === "alias" || objectKind === "global")) {
    ...
    if (assign) {
      ...
      value =
        parsed.kind === "select_expr"
          ? parsed.expr
          : parsed.kind === "select_free"
            ? { kind: "free_object_constructor", entries: parsed.entries }
            : undefined;
    }
  }
  ...
  const functionDecl =
    action === "create" && objectKind === "function"
      ? parseFunctionDecl(source, input, i)
      : undefined;
  ...
  return {
    ...
    ...(alterTypeOps ? { alterTypeOps } : {}),
    pos,
  };
}

function parseConfigureGrammar(
  source: string,
  lexical: readonly Token[],
  lineStarts: readonly number[],
): ConfigureStatement {
  ...
  if (operation === "set" && stream[i]?.kind === "assign") {
    const expressionSource = source
      .slice(stream[i].offset + stream[i].lexeme.length)
      .replace(/;\s*$/, "")
      .trim();
...
function parseGlobalSettingGrammar(
  ...
  if (operation !== "set" && operation !== "reset")
    throw syntaxError("Expected SET or RESET GLOBAL");
  let i = 2;
  const targetStart = i;
...
function hasTopLevelDmlAfterWith(lexical: readonly Token[]): boolean {
  ...
  for (const token of lexical) {
    ...
    if (depth === 0 && ["kw_insert", "kw_update", "kw_delete", "kw_for"].includes(token.kind))
      return true;
...
export function parseEdgeQLGrammar(source: string, defaultModule?: string): Statement {
  ...
  if (lexical[0]?.kind === "kw_configure")
    return parseConfigureGrammar(source, lexical, lineStarts);
  ...
  const input = lexical
    ...
    .map((token) => {
      ...
      if (!type)
        throw syntaxError(
          `Unsupported token '${token.lexeme}' in grammar-backed parser`,
          pos.line,
          pos.column,
        );
      return createTokenInstance(
        ...
        token.lexeme,
        token.offset,
        token.offset + token.lexeme.length - 1,
        pos.line,
        pos.line,
        pos.column,
        pos.column + token.lexeme.length - 1,
      );
    });
  ...
  try {
    const result =
      lexical[0]?.kind === "kw_with" && hasTopLevelDmlAfterWith(lexical)
        ? grammar.withDmlStatement()
        : lexical[0]?.kind === "kw_with" && hasTopLevelGroupAfterWith(lexical)
          ? grammar.groupStatement()
          : lexical[0]?.kind === "kw_group"
            ? grammar.groupStatement()
            : lexical[0]?.kind === "kw_insert"
              ? grammar.insertStatement()
              : lexical[0]?.kind === "kw_update"
                ? grammar.updateStatement()
                : lexical[0]?.kind === "kw_delete"
                  ? grammar.deleteStatement()
                  : lexical[0]?.kind === "kw_for"
                    ? grammar.forStatement()
                    : lexical[0]?.kind === "kw_if"
                      ? grammar.ifStatement()
                      : grammar.statement();
...
export function parseEdgeQLGrammarScript(source: string, defaultModule?: string): Statement[] {
  ...
  const parsePiece = (end: number): void => {
    ...
    const first = lexical.find(
      (token) => token.offset >= start && token.offset < end && token.kind !== "semi",
    );
    if (first) {
      const pieceTokens = lexical.filter(
        (token) =>
          token.offset >= start &&
          token.offset < end &&
          token.kind !== "semi" &&
      ...
      statement.pos = {
        ...
        column:
          statement.pos.line === localStart.line
