...
const reduceSimpleShape = (node: GelCSTNode, source: string): ShapeElement[] => {
  ...
  return elements.map((element) => {
    if (element.name[1] !== "reduce_ShapeElementWithSubShape") {
      throw syntaxError(
        "Generated shape reducer currently supports plain field entries only",
...
export function reduceGelGrammarCST(
  ...
  if (
    resultNode?.kind !== "production" ||
    resultNode.name[0] !== "OptionallyAliasedExpr" ||
    resultNode.name[1] !== "reduce_Expr"
  ) {
    throw syntaxError(
      "Generated CST reducer does not yet support SELECT result aliases",
      startOffset(select),
      source,
    );
  }
  const unsupportedClauses = select.args
    .slice(3)
    .filter(isProduction)
    .filter((clause) => clause.name[1] !== "reduce_empty");
  if (unsupportedClauses.length > 0) {
    throw syntaxError(
      "Generated CST reducer does not yet support SELECT clauses",
      startOffset(select),
      source,
    );
  }
  ...
  if (!expressionRoot)
    throw syntaxError("SELECT is missing its result expression", select.span?.start ?? 0, source);
  ...
  const shape =
    expr.kind === "shape_projection" && expr.expr.kind === "select"
      ? expr.shape
      : subject.kind === "select"
        ? subject.shape
        : undefined;
  if (subject.kind === "select" && shape && (expr.kind === "shape_projection" || filter)) {
