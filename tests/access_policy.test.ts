...
describe("resolveGlobalValue", () => {
  it("resolves a declared global by exact name", () => {
    expect(
      resolveGlobalValue(ctx({ globals: { "default::current": "u1" } }), "default::current"),
    ).toBe("u1");
  });
...
describe("evaluateCondition", () => {
  it("evaluates literal and field conditions", () => {
    ...
    expect(
      evaluateCondition({ kind: "field_eq_literal", field: "n", value: 5 }, { n: 5 }, ctx()),
    ).toBe(true);
    expect(
      evaluateCondition({ kind: "field_eq_literal", field: "n", value: 5 }, { n: 6 }, ctx()),
    ).toBe(false);
  });
  it("matches a field against a global", () => {
    ...
    expect(
      evaluateCondition(
        { kind: "field_eq_global", field: "owner", global: "user" },
        { owner: "u1" },
        c,
      ),
    ).toBe(true);
  ...
  it("conjoins clauses with `and`", () => {
    const cond: AccessPolicyCondition = {
      kind: "and",
      clauses: [
        { kind: "always", value: true },
        { kind: "field_eq_literal", field: "n", value: 1 },
      ],
    };
...
describe("evaluatePoliciesForOperation", () => {
  ...
  it("denies when no relevant policy applies", () => {
    expect(
      evaluatePoliciesForOperation(
        typeWith([policy({ operations: ["insert"] })]),
        "select",
        {},
        ctx(),
        opts,
  ...
  it("requires an allow to match and lets a deny veto it", () => {
    const allowOwner = policy({
      effect: "allow",
      operations: ["select"],
      condition: { kind: "field_eq_global", field: "owner", global: "u" },
    });
    const denyHidden = policy({
      name: "hide",
      effect: "deny",
      operations: ["select"],
      condition: { kind: "field_eq_literal", field: "hidden", value: true },
    });
    const c = ctx({ globals: { u: "u1" } });
    const t = typeWith([allowOwner, denyHidden]);
    expect(evaluatePoliciesForOperation(t, "select", { owner: "u1", hidden: false }, c, opts)).toBe(
      true,
    ); // allowed, not denied
    expect(evaluatePoliciesForOperation(t, "select", { owner: "u1", hidden: true }, c, opts)).toBe(
      false,
    ); // deny vetoes
    expect(evaluatePoliciesForOperation(t, "select", { owner: "u2", hidden: false }, c, opts)).toBe(
      false,
    ); // no allow matches
  });
});
...
describe("enforceInsertPolicies", () => {
  it("throws when the insert is not allowed", () => {
    ...
    expect(() => enforceInsertPolicies(t, {}, ctx(), 1, 1)).toThrow(
