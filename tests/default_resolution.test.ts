...
import {
  applyPendingInsertDefaults,
  type InsertDefaultDeps,
} from "../src/runtime/default_resolution.js";
...
describe("applyPendingInsertDefaults", () => {
  it("fills a literal default", () => {
    ...
    applyPendingInsertDefaults(
      ...
      baseDeps({
        subjectType: typeWithFields([
          { name: "n", hasDefault: true, defaultExpr: { kind: "literal", value: 7 } },
        ]),
      }),
    );
        ...
        subjectType: typeWithFields([
          {
            name: "ts",
            hasDefault: true,
            defaultExpr: { kind: "function_call", name: "std::datetime_current", args: [] },
          },
        ]),
        evalFunctionCall,
      }),
  ...
  it("fills a SQL-text default from the single scalar row evalSelect returns", () => {
    ...
    applyPendingInsertDefaults(
      ...
      baseDeps({
        subjectType: typeWithFields([
          { name: "c", hasDefault: true, defaultExprText: "(SELECT count(T))" },
        ]),
        evalSelect: () => [42],
      }),
    );
        ...
        subjectType: typeWithFields([
          {
            name: "n",
            hasDefault: true,
            defaultExpr: { kind: "function_call", name: "f", args: [] },
          },
        ]),
        evalFunctionCall,
      }),
  ...
  it("re-resolves a column still holding the rewrite-pending sentinel", () => {
    ...
    applyPendingInsertDefaults(
      ...
      baseDeps({
        subjectType: typeWithFields([
          { name: "n", hasDefault: true, defaultExpr: { kind: "literal", value: 3 } },
        ]),
      }),
    );
  ...
  it("leaves a __source__ default pending when the referenced field is unresolved", () => {
    ...
    applyPendingInsertDefaults(
      ...
      baseDeps({
        subjectType: typeWithFields([
          { name: "b", hasDefault: true, defaultExprText: "__source__.a + 1" },
        ]),
