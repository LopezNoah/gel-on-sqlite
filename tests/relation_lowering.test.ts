...
const aggregateInspector = inspectorFor(schemaFromSdl("type Publication { property title: str; }"));

const sql = (query: string, selectedInspector = inspector): string => {
  const result = selectedInspector.inspect(query);
  if (!result.ok)
    throw new Error(`did not compile: ${result.error?.code} ${result.error?.message}`);
  return result.sql();
};
...
describe("Relation-backed path resolution", () => {
  ...
  it("resolves a SET OF aggregate argument against the current Relation scope", () => {
    const out = sql("SELECT Publication.title ?= <str>count(Publication)", aggregateInspector);
  ...
  it("preserves namespaces when correlating aggregate arguments", () => {
    const out = sql("WITH P := Publication SELECT P.title ?= <str>count(P)", aggregateInspector);
