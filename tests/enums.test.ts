...
describe("EdgeQL Enums (SPEC-PARITY)", () => {
  ...
  it("test_edgeql_enums_cast_01", () => {
    h.assertQueryResult(`SELECT <color_enum_t>{'RED', 'GREEN', 'BLUE'};`, ["RED", "GREEN", "BLUE"]);
  });

  it("test_edgeql_enums_cast_02_invalid", () => {
    expect(() => h.query(`SELECT <color_enum_t>'YELLOW';`)).toThrow(/invalid input value for enum/);
  });

  it("test_edgeql_enums_cast_03_case_sensitive", () => {
    expect(() => h.query(`SELECT <color_enum_t>'red';`)).toThrow(/invalid input value for enum/);
  });

  it("test_edgeql_enums_cast_04", () => {
    h.query(`INSERT default::Foo { color := 'BLUE' };`);
    expect(() => h.query(`SELECT 'The test color is: ' ++ default::Foo.color;`)).toThrow(
  ...
  it("test_edgeql_enums_cast_05", () => {
    h.query(`INSERT default::Foo { color := 'BLUE' };`);
    const res = h.query(`SELECT 'The test color is: ' ++ <str>default::Foo.color;`);
  ...
  it("test_edgeql_enums_pathsyntax_01", () => {
    ...
    expect(() => h.query(`WITH e := color_enum_t SELECT e.RED`)).toThrow(
      /enum path expression lacks an enum member name/,
    );
    ...
    expect(() => h.query(`SELECT color_enum_t.<RED`)).toThrow(/enum types do not support backlink/);

    expect(() => h.query(`SELECT color_enum_t[IS color_enum_t].RED`)).toThrow(
    ...
    expect(() => h.query(`WITH x := color_enum_t.RED SELECT x.GREEN`)).toThrow(
      /invalid property reference on an expression of primitive type/,
    );

    expect(() => h.query(`SELECT color_enum_t.RAD`)).toThrow(/enum has no member called 'RAD'/);
  });
  ...
  it("test_edgeql_enums_json_cast_02", () => {
    expect(() => h.query(`SELECT <color_enum_t><json>'BANANA';`)).toThrow(
      /invalid input value for enum.*color_enum_t.*BANANA/,
    );
  });
