...
describe("stdlib registry — SQL lowerability is the sql slot", () => {
  ...
  it("matches the historical SQL-lowerable set", () => {
    ...
    const actual = STDLIB_FUNCTIONS.filter((e) => e.sql)
      .map((e) => e.name)
      .sort();
...
describe("stdlib registry — runtime dispatch", () => {
  ...
  it("shares the unpack body across range_unpack and array_unpack", () => {
    expect(executeStdlibFunction("std::array_unpack", [{ kind: "array", values: [1, 2] }])).toEqual(
      [1, 2],
    );
    expect(executeStdlibFunction("std::range_unpack", [{ kind: "array", values: [1, 2] }])).toEqual(
      [1, 2],
    );
  });
