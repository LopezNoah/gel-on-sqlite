...
const lit = (value: unknown, numericKind?: string) => ({
  kind: "literal",
  value,
...
describe("inferArrayValuedType", () => {
  it("infers the element type of a non-empty array literal", () => {
    expect(inferArrayValuedType({ kind: "array_literal", values: [lit(1)] })).toEqual({
      kind: "array",
      element: "std::int64",
    });
    expect(inferArrayValuedType({ kind: "array_literal", values: [lit("x")] })).toEqual({
      kind: "array",
      element: "std::str",
    });
  });
  it("treats a bare empty array as indeterminate", () => {
    expect(inferArrayValuedType({ kind: "array_literal", values: [] })).toEqual({
      kind: "indeterminate",
    });
  });
  it("takes the element type from a non-empty operand of a ++ concat", () => {
    expect(
      inferArrayValuedType({
        kind: "concat",
        parts: [
          { kind: "array_literal", values: [] },
          { kind: "array_literal", values: [lit(1)] },
        ],
      }),
  ...
  it("peels an `expr` wrapper", () => {
    expect(
      inferArrayValuedType({ kind: "expr", expr: { kind: "array_literal", values: [lit(true)] } }),
...
describe("computedExprIsMulti", () => {
  it("is true for a >1 set literal/expr, peeling select wrappers", () => {
    ...
    expect(
      computedExprIsMulti({ kind: "select_expr", expr: { kind: "set_expr", values: [1, 2, 3] } }),
    ).toBe(true);
  });
...
describe("unwrapSubqueryWrappers", () => {
  it("peels subquery wrappers to the inner node", () => {
    ...
    expect(
      unwrapSubqueryWrappers({
        kind: "subquery_expr",
        expr: { kind: "select_expr_subquery", expr: inner },
      }),
    ).toBe(inner);
  });
...
describe("bindingSelectShape", () => {
  ...
  it("reads the shape from a subquery_expr binding", () => {
    const binding = {
      kind: "subquery_expr",
      expr: { kind: "select", shape: [{ kind: "field", name: "a" }] },
    } as unknown as WithBindingValue;
...
describe("insertValueHasUnscopedPartialPath", () => {
  it("is true for a bare current-item reference", () => {
    ...
    expect(
      insertValueHasUnscopedPartialPath({ kind: "tuple", items: [{ kind: "current_item" }] }),
    ).toBe(true);
  });
  it("does not descend into a nested query scope", () => {
    expect(
      insertValueHasUnscopedPartialPath({ kind: "select", shape: [{ kind: "current_item" }] }),
    ).toBe(false);
  });
