...
The domain language of the EdgeQL→SQLite query engine. Terms here are specific to _this_ rebuild's pipeline and to Gel's semantics — not general compiler vocabulary. Sharpened during the 2026-06-16 architecture review (see `docs/adr/`).
...
**DML IR**:
The mutation-only IR (`InsertIR` / `UpdateIR` / `DeleteIR` and their link sub-IRs) emitted by `src/compiler/dml_lowering.ts` and consumed by the engine's write path. `src/ir/model.ts` is now _only_ this IR: the dead SELECT/GROUP statement IR (`SelectIR` / `SelectFreeIR` / `SelectExprIR` / `GroupIR`) and their exclusive supporting types — shape/filter/order-by/expression-atom IR, ~600 lines — were deleted once the two-IR era ended (see `docs/adr/0021`). `CompileArtifact.ir` is the DML IR for mutations and `undefined` for SELECT / GROUP / FOR (which carry no IR — they run off the Live IR's SQL artifact).
...
**Concrete-type closure**:
The set of **concrete** (non-abstract) types a given type covers — itself if concrete, plus every concrete subtype. The schema home is `src/schema/schema.ts`: `listConcreteTypesAssignableTo(name): TypeDef[]` is the primitive walk; `concreteTypeNamesUnder(name): string[]` is its qualified-name projection (`listConcreteTypesAssignableTo(name).map(qualifiedTypeName)`), and membership ("is concrete type `X` covered by `name`?") is `concreteTypeNamesUnder(name).includes(X)`. The projection used to be re-spelled inline at ~16 sites across `engine.ts` / `evaluator.ts` / `ast_to_ir.ts` / `inference.ts` / `dml_lowering.ts` / `service.ts` (each `.map(qualifiedTypeName)` or `.some(c ⇒ qualifiedTypeName(c) === X)`); they now route through `concreteTypeNamesUnder`. The SQL layer reaches this same closure across the compile seam via the `resolveConcreteSubtypes` option (`compiler_types.ts`), wired in `service.ts` to `concreteTypeNamesUnder` — see the [[Polymorphic source]] entry for the SQL-side reader (`compilePolymorphicSource`), where a union typeref arrives as a bare `A | B` id with no `children` and must recover its branches' subtypes through this resolver.
_Avoid_: subtype list, type closure, assignable types.
...
**Polymorphic source**:
`src/sql/gel_ir_compiler.ts::compilePolymorphicSource(typeRef, …)` — the SQL-side reader of the [[Concrete-type closure]]: the `(SELECT … FROM <concrete table> UNION ALL …)` over every concrete type a possibly-abstract/union typeref admits, surfacing a uniform `__source_type` discriminator and projected columns (42 call sites). A union typeref arrives as a bare `A | B` id string with **no `children`**, so the per-branch type-closure would miss each branch's own subtypes — recovered through the `resolveConcreteSubtypes` option (wired in `service.ts` to `concreteTypeNamesUnder`). The per-branch "expand to concrete extent" rule — schema resolver when wired, else the IR children-closure (`flattenTypeClosure`) — has one home in `branchConcreteTypeNames`, shared with `linkStorageTablesForPointer` (the distinct junction tables a polymorphic link read must union over) so the two cannot drift. `compilePolymorphicSource` keeps the children-closure for **non-union** sources to hold its UNION-ALL branch order byte-identical; only the union path consults the schema. `typeRefFromQualifiedName` synthesises the minimal concrete TypeRef from a recovered name. The runtime twin is [[Runtime type narrowing]].
_Avoid_: union source, poly source, type-closure SQL.
...
**Runtime type narrowing** (`src/runtime/type_narrowing.ts`):
The runtime twin of the [[Polymorphic source]] — EdgeQL's `[IS T]` intersection, `IS` type test, the `__type__`-style discriminator read, and polymorphic field access, as evaluated by the [[Runtime evaluator]]. `evalTypeNarrowing(expr, env, deps)` handles the four `evalExpr` cases (`type_intersection` / `is_type` / `type_name` / `polymorphic_field_ref`) that read a row's `__source_type` and gate on the [[Concrete-type closure]] (`schema.concreteTypeNamesUnder`) — so the runtime and the SQL path agree on what a `[IS T]` admits. Lifted out of the 39-case `evalExpr` behind a small injected seam (`evalExpr` for recursion, `schema`, `qualifyRuntimeTypeName`) so `[IS T]` behaviour has its own interface, drivable with a stub `evalExpr` and a real schema — no DB, no full evaluator. The **rest** of the `evalExpr` decomposition stays deferred (`docs/adr/0014`); this is the one slice carved because narrowing is the active failing-test cluster. Pinned by `tests/type_narrowing.test.ts`.
_Avoid_: IS check, discriminator eval, the narrowing pass.

**Access-policy enforcement** (`src/runtime/access_policy.ts`):
The one home for the policy decision — "does this security context pass the access policies declared on this type for this operation?". `evaluatePoliciesForOperation` is the core rule (superuser bypass, no-relevant-policy ⇒ deny, allow-any-then-deny-veto); `appliesToOperation` matches an operation (`update` ⇒ `update_read`+`update_write`); `evaluateCondition` evaluates one condition; `resolveGlobalValue`/`hasPermission` resolve globals/permissions; the four `enforce{Insert,UpdateRead,UpdateWrite,Delete}Policies` throw-on-deny wrappers are what the write path calls. The read-time check `evaluateSelectPolicies` stays in `engine.ts` (it needs the shared `rowSourceType`/`readRowById` row helpers) but delegates its decision here. Note: `runWriteWithAccessPolicies` (engine.ts, ~575 lines) is a write _executor_ that merely calls these — its size is mutation mechanics (INSERT defaults, sequences, `UNLESS CONFLICT`, SQL assembly), not policy logic. Pinned by `tests/access_policy.test.ts` (`docs/adr/0037`).
...
**Write-time DML SQL emission** (`src/runtime/dml_sql.ts`):
`buildInsertRowSql(table, entries, insertColumns, pos)` — the pure emitter for the INSERT row SQL. UPDATE/DELETE compile to SQL ahead of time (DML IR → `sqlArtifact.sql` via `dml_lowering.ts`), but INSERT's column set is known only at write time (after defaults, sequences, and inline-link targets resolve), so its SQL is emitted here. Three shapes: `DEFAULT VALUES` (no columns), positional `?` params (boolean→1/0), and splicing a planner-deferred column's compiled expression (`PENDING_INSERT_SQL_EXPR_VALUE`) + its param slice. Pure (`quoteIdent` + the sentinel + `AppError`, no db/schema/context), lifted out of `runWriteWithAccessPolicies` for a name and a test surface — **not** a DML compiler: the rest of the write executor (sequences, defaults, `UNLESS CONFLICT`, link/multi-scalar writes, policies) is irreducible write-time mutation mechanics (`docs/adr/0037`), and UPDATE/DELETE need no emitter. Pinned by `tests/dml_sql.test.ts` (`docs/adr/0046`). The stub-SQL/runtime-probe in `executeForLoop` was reviewed and is **not** a routing-authority violation (`docs/adr/0045`).
_Avoid_: DML compiler, insert builder.

**Exclusive-conflict detection** (`src/runtime/conflict_detection.ts`):
The `UNLESS CONFLICT` machinery, lifted from the write executor (continuing the carve of access-policy / default-resolution / dml_sql — `docs/adr/0063`). Emitter-style ("plan + run"): `exclusiveChecksFor(schema, type, targetFields?)` enumerates the exclusive constraints reachable from a type (field-level / multi / type-level tuple / inherited / `except` / the implicit id PK) — PURE given the schema; `planExclusiveConflictProbe(check, values)` is the PURE emitter deciding what to probe (or `null` when a covered value is absent, since an empty set can't clash); `runExclusiveConflictProbe(db, plan)` is the thin DB step that gates on `PRAGMA table_info` then runs the probe (byte-identical to the old inline `findExclusiveConflictId`). Plus `parseExclusivityViolation` (recover the violated property from a SQLite UNIQUE error), `conflictIsAgainstSameStatementRow` (the same-statement-clash guard, arg narrowed from `SecurityContext` to `statementInsertedIds`), and `insertValueIsVolatile` (the volatile-conflict-target guard). The module OWNS the exclusivity primitives — `engine.ts`'s WITH-DML-chain exclusivity snapshot imports `exclusiveChecksFor` / `typeAncestorsOf` / `constraintIsExclusiveLike` back (one-directional, no cycle; single home, not a duplicate). The plan is the test surface every constraint kind crosses with NO DB write (pinned by `tests/conflict_detection.test.ts`). Why plan+run and not a pure SQL string: the probe reads `PRAGMA table_info` at run time, so only the plan is pure; the introspection-gated execution stays DB-coupled.
_Avoid_: conflict checker, unique-violation handler.
...
**Co-iteration**:
`src/runtime/co_iteration.ts` — EdgeQL's rule that two operands walking the _same_ set-valued WITH binding iterate in lockstep, not as a Cartesian product (`WITH x := {1,2,3} SELECT x * x` ⇒ {1,4,9}). `findBindingRoot` finds the binding a path is rooted in; `coIteratedBinding(left, right, env)` is the shared detection used by the evaluator's `math` and `compare` cases. The per-row bodies stay in those cases (math applies the operator; compare does LCP `?=`/`?!=`) — only the detection is shared. A first, safe slice of making the Runtime evaluator testable; the full `evalExpr` extraction is deferred (see `docs/adr/0014`).
_Avoid_: lockstep, zip.

**SQL-precompute probe**:
`tryRunSingleSqlRows` in `src/runtime/engine.ts` — "compile this statement; if it lowers to a single SQL statement, run it and return the rows, else `undefined`" — the shared home for "try SQL, else fall back to the Runtime evaluator". Built on `tryProbe` (`src/errors.ts`), so a query problem (`isQueryFailure`) falls back but an engine defect (TypeError, E_RUNTIME) propagates rather than hiding behind a bare `catch`. Used by computed-global defaults and FOR scalar iterators. Other swallowing catches (raw-SQLite missing-column probes, JSON-decode fallbacks, parse retries) are deliberately _not_ routed through it — there the swallowed error is expected, not a defect (see `docs/adr/0012`).
