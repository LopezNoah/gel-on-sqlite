...
The home for computing the inference facts on the Live IR, so the [[inference oracle]] (`semantic.ts`) can eventually be deleted (the ADR 0001 follow-up). Implements **volatility** (`inferStatementVolatility`, ADR 0015) and **cardinality** (`inferStatementCardinality`, ADR 0016) — both ported from the oracle and invoked by `compileASTToGelIR`; the Live IR now matches the oracle on all 17 volatility cases and all 121 statement-level cardinality cases (`tests/edgeql_ir_volatility_live.test.ts`, `tests/edgeql_ir_card_live.test.ts`). All statement-level inference is **additive** — execution reads only per-element/per-`Set` cardinality, never the statement-level `cardinality`/`multiplicity`/`stype`/`scopeTree` fields — and each call is `try/catch`-guarded so a pathological AST can never break a compile. Remaining for oracle deletion: **multiplicity** (~105 cases, builds on cardinality), **type** (6 cases, incl. derived union types), **scope-tree** (6 cases — note: these are *error-detection*, not a value, so non-additive), plus 33 shape-element cardinality cases that belong to the SQL builder.
