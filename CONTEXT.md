...
The `UNLESS CONFLICT` machinery, lifted from the write executor (continuing the carve of access-policy / default-resolution / dml_sql — `docs/adr/0063`). Emitter-style ("plan + run"): `exclusiveChecksFor(schema, type, targetFields?)` adapts the shared constraint description from `src/schema/exclusive_constraints.ts::exclusiveConstraintFactsForType` to the probe form; `planExclusiveConflictProbe(check, values)` is the PURE emitter deciding what to probe (or `null` when a covered value is absent, since an empty set can't clash); `runExclusiveConflictProbe(db, plan)` gates on `PRAGMA table_info` then runs the probe (byte-identical to the old inline `findExclusiveConflictId`). Plus `parseExclusivityViolation`, `conflictIsAgainstSameStatementRow`, and `insertValueIsVolatile`. The shared schema description owns inheritance coverage, delegated per-type scope, column mapping, case-folding, and the supported single-field `except` interpretation; both this module and `schema_materialize.ts` consume it. `engine.ts`'s WITH-DML-chain exclusivity snapshot still imports `exclusiveChecksFor` / `typeAncestorsOf` / `constraintIsExclusiveLike` from here. The plan remains the test surface every constraint kind crosses with NO DB write (pinned by `tests/conflict_detection.test.ts`). Why plan+run and not a pure SQL string: the probe reads `PRAGMA table_info` at run time, so only the plan is pure; the introspection-gated execution stays DB-coupled.
